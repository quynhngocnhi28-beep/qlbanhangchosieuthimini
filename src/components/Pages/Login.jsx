...
const Login = () => {
    ...
    // Load tài khoản đã nhớ khi trang load[cite: 14]
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (matchedAccount) {
                // Tích hợp tính năng bảo mật và điều hướng[cite: 17]
                const publicInfo = { ...matchedAccount };
                delete publicInfo.pass; // Xóa mật khẩu khỏi đối tượng lưu trữ[cite: 17]
                localStorage.setItem('currentUser', JSON.stringify(publicInfo));
                window.dispatchEvent(new Event('userUpdated'));

                // Điều hướng theo vai trò[cite: 17]
                ...
                {forgotMode ? (
                    <>
                        ...
                        <form className="login-form" onSubmit={handleForgotSubmit}>
                            ...
                            <input type="password" className="form-input" placeholder="Mật khẩu mới" value={fpNew} onChange={(e) => setFpNew(e.target.value)} />
                            <input type="password" className="form-input" placeholder="Xác nhận mật khẩu mới" value={fpConfirm} onChange={(e) => setFpConfirm(e.target.value)} />
                            {fpError && <div className="login-error">{fpError}</div>}
                            {fpSuccess && <div className="login-success" style={{ color: 'green', textAlign: 'center' }}>{fpSuccess}</div>}
